...
/**
 * Response from requesting presigned upload URL
 */
...
export async function getUploadUrl(
    ...
    try {
        const response = await apiClient.put<UploadUrlResponse>(
        ...
        // Validate response
        if (!response.data || !response.data.upload_url || !response.data.object_key) {
 ...
 * @returns Promise with material_id
 */
export async function uploadMaterial(
    ...
    try {
        ...
        console.log('[uploadMaterial] 🎉 Complete upload flow finished! Material ID:', result.material_id);
