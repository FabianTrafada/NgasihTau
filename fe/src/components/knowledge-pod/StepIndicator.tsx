      ...
      {steps.map((step, index) => {
        ...
        return (
          <div key={index} className="relative flex items-center gap-6 group">
            ...
            {!isLast && (
              <div
                className={`absolute left-[15px] top-[32px] w-[2px] h-[40px] z-0 transition-colors duration-500 ${isCompleted ? 'bg-[#FF8A00]' : 'bg-gray-100'
                  }`}
              />
            )}
            ...
            <div
              className={`relative z-10 w-8 h-8 rounded-full border-2 transition-all duration-500 flex items-center justify-center font-black text-[10px] shrink-0 ${isCompleted
