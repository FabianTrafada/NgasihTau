...
import { getPodBySlug, getPodMaterials } from "@/lib/api/pod";
...
export default function KnowledgePodDetail({ params }: PageProps) {
  ...
  const { username, pod_slug } = React.use(params);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Fetching pod with slug:", pod_slug);
        console.log("URL params - username:", username, "pod_slug:", pod_slug);

        // Fetch pod detail by slug
        const podData = await getPodBySlug(pod_slug);
        ...
        try {
          const podMaterials = await getPodMaterials(podData.id);
